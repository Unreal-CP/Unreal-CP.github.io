...
# 前言
将讲解Lyra是如何通过数据驱动相机系统，并与传统的弹簧臂组件(SpringArmComponent)进行一定的区别
但具体到开发实现上，例如射击游戏中的腰射/机瞄/倍镜/固定武器，ACT(Action Game)中的自由/锁定视角中，真实的需求只是通过同样的参数组去实现不同的相机逻辑。以及如果需要在Runtime中频繁切换组件层级Object未免有些繁琐。相较之下，用不同的模式去解耦这些逻辑会更加灵活。

# 调试命令
``` cmd 
ShowDebug CAMERA 
```
![DebugCamera](./assets/img/Pictures/相机/调试.png)
>请用编辑器得下窗口运行改命令,如果以`新建编辑器窗口（PIE）` 运行会导致崩溃.
{: .prompt-danger }

# 相机管理机制
## 引擎默认的相机系统
![默认相机流程图](./assets/img/Pictures/相机/默认相机流程图.png)
>`PlayerCameraManager`中有个很重要的函数是`UpdateViewTarget`，只要重载这个函数就可以接管Gameplay中Camera的管理。
{: .prompt-tip }

- 输入改变`ControlRotation`;
- `SpringArmComponent`使用`ControlRotation`改变`ChildActor(即CameraComponent)`的`RelativeTransform`;
- `CameraManager`在计算最终**相机POV**[^相机POV] 时最终调用的是**Pawn身上挂载**的`CameraComponent`的`CameraComponent::GetCameraView`函数；
>CameraComponent中的重要函数是GetCameraView,重载这个函数可以做很多Gameplay相关的逻辑或者设计，Lyra正是利用了这一点。
{: .prompt-tip }

`SpringArmComponent`的核心在于`UpdateDesiredArmLocation`函数，这个函数会计算相机最终的位置和朝向，如果设置了`bUsePawnControlRotation`那么会使用`ControlRotation`, 而最终位置就需要使用`CollisionTest`的函数，
比如`SweepSingleByChannel`，如果发生了碰撞，那么最终的位置跟碰撞点有关，如下图所示：
![弹簧臂](./assets/img/Pictures/相机/弹簧臂.jpg)


## Lyra相机系统
在Lyra中，实现了分层的相机管理机制。
```
ULyraCameraComponent : 组件类，挂载到Character上，接受外部委托设置CameraMode。
   |
ULyraCameraModeStack : 管理Mode生命周期，提供Mode混合功能
   |
ULyraCameraMode :  专注于单个相机逻辑实现
```
**管理层 (Management Layer)**
```cpp
ALyraPlayerCameraManager (管理者)
├── ULyraUICameraManagerComponent (UI相机)
└── ULyraCameraComponent (游戏相机)
```
**模式层 (Mode Layer)**
```cpp
ULyraCameraModeStack (模式堆栈管理器)
├── ULyraCameraMode (抽象基类)
│   ├── ULyraCameraMode_ThirdPerson (具体实现)
```
**流程图**
![Lyra相机系统流程图](./assets/img/Pictures/相机/Lyra相机系统流程图.png)
1. `PawnReady`的时候会设置`Input, Ability，Camera`相关的设置，这里**只是注册了**回调函数`DetermineCameraMode`，这个函数每帧会告诉`LyraCameraComponent`当前应该使用哪个CameraMode配置
2. 每帧更新相机`POV(PointofView)`信息时都会从`PlayerCameraManager::UpdateViewTarget`开始
3. 通过上面的CameraSystem流程图可以知道，**POV的结果**由角色身上`CameraComponent::GetCameraView`函数返回
4. `GetCameraView`会调用到`UpdateCameraModes`
   - 这个函数会**调用第1步时注册的代理函数**，LyraCameraComponent就知道了这一帧要使用哪个CameraMode配置
5. 第4步会返回CameraMode的Class, 这一步会`NewObject`这个`CameraMode`(如果还没有创建过)，并将这个实例放入`CameraModeStack`的**栈顶**
6. `CameraModeStack`内部**可能有多个**`CameraMode`, 这一步会遍历所有的CameraMode并得到每一个CameraMode返回的POV, 再结合每个CameraMode的BlendWeight计算最终的POV
7. 得到最终的POV后，将`PlayerController`的`ControlRotation`设置为`返回的ControlRotation`，同样将   `PlayerComponent`的**世界坐标**设置为返回的结果，这样`POV, ControlRotation和CameraComponent`的`Transform`数据一致
8. CameraSystem流程图中的后续流程，比如[ViewTargetBlend](#混合权重过程)等等。



# APlayerCameraManager
```cpp
// 一个“玩家摄像机管理器”负责管理特定玩家的摄像机。
// 它定义了其他系统（例如渲染器）所使用的最终视图属性，这意味着你可以将其视为存在于世界中的虚拟眼球。
// 它可以直接计算最终的摄像机属性，也可以在其他影响摄像机的对象或角色之间进行仲裁/混合（例如从一个摄像机角色过渡到另一个）。
// 玩家摄像机管理器的主要外部职责是可靠地响应各种“获取*”函数，例如“获取摄像机视点”。其余的一切都是实现细节，可由用户项目进行重写。
// 默认情况下，一个玩家摄像机管理器会维护一个“视图目标”，即摄像机所关联的主要角色。
// 它还可以对最终视图状态应用各种“后置”效果，例如摄像机动画、抖动、后期处理效果或诸如镜头上的污渍之类的特殊效果。
```
``` cpp
/**
...
```
>这个类主要核心就在于它得**UpdateViewTarget**函数.在这函数中会去调用找到得相机组件得**GetCameraView**.这个就是我们介入得地方
{: .prompt-info }
``` cpp
...
void APlayerCameraManager::UpdateViewTarget(FTViewTarget& OutVT, float DeltaTime)
{
	...
	OutVT.POV.FOV = DefaultFOV;					// 设置默认视野角度
	OutVT.POV.OrthoWidth = DefaultOrthoWidth;	// 设置正交投影宽度
	OutVT.POV.AspectRatio = DefaultAspectRatio;	// 设置屏幕宽高比
	OutVT.POV.bConstrainAspectRatio = bDefaultConstrainAspectRatio; // 是否强制宽高比
	OutVT.POV.ProjectionMode = bIsOrthographic ? ECameraProjectionMode::Orthographic : ECameraProjectionMode::Perspective; // 投影模式：正交/透视
	OutVT.POV.PostProcessBlendWeight = 1.0f;	// 后处理混合权重
	OutVT.POV.bAutoCalculateOrthoPlanes = bAutoCalculateOrthoPlanes; // 是否自动计算正交平面
	OutVT.POV.AutoPlaneShift = AutoPlaneShift;	// 自动平面偏移量
	OutVT.POV.bUpdateOrthoPlanes = bUpdateOrthoPlanes; // 是否更新正交平面
	OutVT.POV.bUseCameraHeightAsViewTarget = bUseCameraHeightAsViewTarget; // 是否使用相机高度作为视图目标高度

	bool bDoNotApplyModifiers = false; // 是否禁用相机修饰器（如震动、偏移等）

	if (ACameraActor* CamActor = Cast<ACameraActor>(OutVT.Target))
	{
		// Viewing through a camera actor.
		// 通过相机Actor观察（直接使用相机Actor的参数）
		CamActor->GetCameraComponent()->GetCameraView(DeltaTime, OutVT.POV);
	}
	else
	{
		// 预定义相机样式名称常量
		static const FName NAME_Fixed = FName(TEXT("Fixed"));			// 固定视角
		static const FName NAME_ThirdPerson = FName(TEXT("ThirdPerson"));	// 第三人称视角
		static const FName NAME_FreeCam = FName(TEXT("FreeCam"));		// 自由相机
		static const FName NAME_FreeCam_Default = FName(TEXT("FreeCam_Default")); // 默认自由相机
		static const FName NAME_FirstPerson = FName(TEXT("FirstPerson"));	// 第一人称视角

		if (CameraStyle == NAME_Fixed)
		{
			// do not update, keep previous camera position by restoring
			// saved POV, in case CalcCamera changes it but still returns false
			// 固定视角模式：不更新视角，恢复之前保存的POV（防止CalcCamera修改但返回false）
			OutVT.POV = OrigPOV;
		...
		else if (CameraStyle == NAME_ThirdPerson || CameraStyle == NAME_FreeCam || CameraStyle == NAME_FreeCam_Default)
		{
			...
			// 简单第三人称视角实现
			FVector Loc = OutVT.Target->GetActorLocation();	// 获取目标Actor位置
			FRotator Rotator = OutVT.Target->GetActorRotation(); // 获取目标Actor旋转

			if (OutVT.Target == PCOwner)
			{
				// 如果目标是所属PlayerController，使用焦点位置（如玩家焦点）
...
//          OutVT.Target.GetActorEyesViewPoint(Loc, Rot);
			...
			{
				// 自由相机模式下，使用PlayerController的控制旋转
				Rotator = PCOwner->GetControlRotation();
			}

			// 计算相机位置：目标位置 + 偏移量 + 自由相机偏移 - 朝向向量 * 相机距离
			FVector Pos = Loc + ViewTargetOffset + FRotationMatrix(Rotator).TransformVector(FreeCamOffset) - Rotator.Vector() * FreeCamDistance;
			FCollisionQueryParams BoxParams(SCENE_QUERY_STAT(FreeCam), false, this); // 碰撞查询参数（用于相机碰撞检测）
			BoxParams.AddIgnoredActor(OutVT.Target); // 忽略目标Actor（防止相机与自身碰撞）
			FHitResult Result; // 碰撞检测结果

			// 执行球形碰撞检测（从目标位置到相机位置）
			GetWorld()->SweepSingleByChannel(Result, Loc, Pos, FQuat::Identity, ECC_Camera, FCollisionShape::MakeBox(FVector(12.f)), BoxParams);
			// 根据碰撞结果设置相机位置：无碰撞则用计算位置，有碰撞则用碰撞点
			OutVT.POV.Location = !Result.bBlockingHit ? Pos : Result.Location;
			OutVT.POV.Rotation = Rotator; // 设置相机旋转
...
![第三人称开镜流程](./assets/img/Pictures/相机/第三人称开镜流程.jpg)

这里计算混合权重为累乘计算，假设Mode栈如下
```cpp
CameraModeA - 0.9
CameraModeB - 0.5
CameraModeC - 1（栈底的权重一定为1）
最后权重
（CameraModeC * 1） * （1 - 0.5） +  CameraModeB * 0.5） *  （1 - 0.9） +  CameraModeA * 0.9
=
CameraModeC * 0.05 + CameraModeB * 0.05 + CameraModeA * 0.9
```
![权重计算过程](./assets/img/Pictures/相机/权重计算过程.png)
反推回去，如果要将CameraModeC提至栈顶，权重
`1 * (1 - 0.9) * (1 - 0.5) = 0.05`
此时，Mode栈更新为
```cpp
CameraModeC - 0.05
CameraModeA - 0.9
CameraModeB - 1（栈底的权重一定为1）
最后权重
（（CameraModeB * 1） * （1 - 0.9） +  CameraModeA * 0.9） *  （1 - 0.05） +  CameraModeC * 0.05
=
CameraModeC * 0.05 + CameraModeA * 0.855 + CameraModeB * 0.195
```
这样做的优点:

- **当某一Mode的权重为1时，可以完全忽略其后的Mode权重**
![混合顺序](./assets/img/Pictures/相机/混合顺序.png)
- `ULyraCameraModeStack`中维护了一个CameraMode栈，每帧Push外部指定的CameraMode类（压到栈顶或从栈中提上来）。
- 再调用这些`CameraMode->UpdateCameraMode(DeltaTime)`更新权重。
- 最后依次调用`CameraMode->GetCameraModeView()`并混合。

期间还会调用CameraMode的生命周期函数
### 推入新的相机模式
``` cpp
...
void ULyraCameraModeStack::GetBlendInfo(float& OutWeightOfTopLayer, FGameplayTag& OutTagOfTopLayer) const
{
	...
	else
	{
		...
		// 在这个过程中,栈底的老的相机模式还在生效,所以外部获取时应该获取基本盘.直到新的完全生效,老的被移除!!!!!
...
# 参考文章
[UE Camera【1.控制流程】](https://zhuanlan.zhihu.com/p/666271135)
[UE Lyra学习【3.Camera】](https://zhuanlan.zhihu.com/p/709665458)
[Lyra中的相机系统](https://zhuanlan.zhihu.com/p/10432602003)

[^相机POV]:POV是`Point of View`的缩写，意思是视角或视点。常见的POV：第一人称、第三人称、俯视视角.
           FOV是`Field of View`的缩写，意思是视野或视场角。它指的是通过镜头（或你的眼睛）所能观察到的世界范围的广度，通常用角度来表示。